module OrdersHelper
  ...
  def collect_fees(response)
    {
      mercadopago_fee: helpers.parse_fee(response, 'mercadopago_fee'),
      application_fee: helpers.parse_fee(response, 'application_fee')
    }
  end
  ...
  def parse_response(response)
    {
      ...
      mp_response: mp_statuses[response['status']][response['status_detail']],
